...
A = foreach data generate faq_id, faq_name, question, answer_type, similarity;
filterAns = filter A by similarity >= $siml and similarity <= $simr;

grpData	= group filterAns by question;
genData	= foreach grpData generate flatten(group) as question, COUNT(filterAns) as count;
odrData	= order genData by $1 desc;

Result = foreach foreachAll generate xiaoiUDFs.GenerateRowKey('stat', 'hotQuestion', '$date'), count, question;
--dump ansTypeResult;
--store ansTypeResult into '$hdfs_output_dir';
store Result into 'shmcc_stat' using org.apache.pig.backend.hadoop.hbase.HBaseStorage('stat_cf:count stat_cf:question');
