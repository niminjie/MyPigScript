...
groupData = foreach (group filterAns all) generate COUNT(filterAns) as visitAll;
genData	= foreach data generate session_id;
disSess	= distinct genData;
grpData	= group disSess all;
result = foreach grpData generate COUNT(disSess) as visitPeople;

foreachAll	= foreach groupData generate (visitAll / (double)result.visitPeople) as count;

ansTypeResult = foreach foreachAll generate xiaoiUDFs.GenerateRowKey('stat', 'avg_visit_volume', '$date'), count;
--dump ansTypeResult;
--store ansTypeResult into '$hdfs_output_dir';
store ansTypeResult into 'shmcc_stat' using org.apache.pig.backend.hadoop.hbase.HBaseStorage('stat_cf:count');
