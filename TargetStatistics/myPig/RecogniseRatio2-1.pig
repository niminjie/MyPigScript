...
groupData	= foreach (group filterAns all) generate COUNT(filterAns) as visit;
filAns	= filter filterAns by answer_type == 1;
genAns		= foreach (group filAns all) generate COUNT(filAns) as cnt;

generateAns	= foreach genAns generate (cnt / (double)groupData.visit);

ansTypeResult = foreach generateAns generate xiaoiUDFs.GenerateRowKey('stat', 'recognise_ratio', '$date'), count;
--dump ansTypeResult;
